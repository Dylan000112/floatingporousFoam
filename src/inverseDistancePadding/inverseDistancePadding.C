...
#include "inverseDistancePadding.H"
...
{
    defineTypeNameAndDebug(inverseDistancePadding, 0);
    addToRunTimeSelectionTable(cellCellStencil, inverseDistancePadding, mesh);
}
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

Foam::label Foam::cellCellStencils::inverseDistancePadding::index
(
...
Foam::labelVector Foam::cellCellStencils::inverseDistancePadding::index3
(
    const labelVector& nDivs,
    const label boxI
)
...
Foam::point Foam::cellCellStencils::inverseDistancePadding::position
(
    const boundBox& bb,
    const labelVector& nDivs,
    const label boxI
...
void Foam::cellCellStencils::inverseDistancePadding::fill
(
    PackedList<2>& elems,
    const boundBox& bb,
    const labelVector& nDivs,
...
/*---------------------------------------------------------------------------*\
  Member Function: markBoundaries

  Description:
    Identifies the overset fringe and marks the internal "core" region as 
    virtual obstacles in the voxel map.
    
    Logic Strategy:
    1. External (Voxel Map): Mark core cells as PATCH to induce hole-cutting 
       in the background mesh.
    2. Internal (Local Map): Keep core cells as OTHER (Fluid) to allow 
       porous media / flow solution on this mesh.
\*---------------------------------------------------------------------------*/

void Foam::cellCellStencils::inverseDistancePadding::markBoundaries
(
    const fvMesh& mesh,
    ...
    const label nPaddingLayers
)
{
    const fvBoundaryMesh& pbm = mesh.boundary();
    
    // 1. Initialization
    // Reset all voxel types to OTHER (Fluid/Calculated)
    patchTypes = patchCellType::OTHER;

    // 2. Identify Overset Seeds (Fringe Interface)
    // ----------------------------------------------------------------------
    labelList cellLevel(mesh.nCells(), -1);
    label nSeeds = 0;
    ...
    forAll(pbm, patchI)
    {
        ...
        if (isA<oversetFvPatch>(fvp))
        {
            ...
            forAll(pp, i)
            {
                ...
                // Mark local status as OVERSET
                patchCellTypes[cellMap[cellI]] = patchCellType::OVERSET;

                // Mark voxel map as OVERSET
                ...
                // Initialize seed for BFS (Level 0)
                if (cellLevel[cellI] == -1) 
                {
                    cellLevel[cellI] = 0;
                    nSeeds++;
                }
    ...
    // 3. Check for Active Foreground Mesh
    // ----------------------------------------------------------------------
    label nGlobalSeeds = nSeeds;
    reduce(nGlobalSeeds, sumOp<label>());

    // If no overset patches are found (e.g., background mesh), 
    // skip the virtual obstacle logic to prevent self-cutting.
    if (nGlobalSeeds == 0)
    {
        return; 
    }
    ...
    if (Pstream::master())
    {
        Info<< "    [HoleCut] Overset fringe initialized. Seeds: " << nGlobalSeeds 
            << ". Propagating " << nPaddingLayers << " layers inwards." << endl;
    }

    // 4. Wavefront Propagation (BFS)
    // ----------------------------------------------------------------------
    // Propagate inwards to establish the interpolation fringe.
    const labelList& own = mesh.faceOwner();
    const labelList& nei = mesh.faceNeighbour();

    for (label k = 0; k < nPaddingLayers; ++k)
    {
        label nChangedLocal = 0;
        labelList nbrCellLevel;
        syncTools::swapBoundaryCellList(mesh, cellLevel, nbrCellLevel);

        // Propagate across internal faces
        for (label faceI = 0; faceI < mesh.nInternalFaces(); ++faceI)
        {
            label cOwn = own[faceI];
            label cNei = nei[faceI];

            if (cellLevel[cOwn] == k && cellLevel[cNei] == -1) {
                cellLevel[cNei] = k + 1;
                nChangedLocal++;
            }
            else if (cellLevel[cNei] == k && cellLevel[cOwn] == -1) {
                cellLevel[cOwn] = k + 1;
                nChangedLocal++;
            }
        }

        // Propagate across processor boundaries
        for (label faceI = mesh.nInternalFaces(); faceI < mesh.nFaces(); ++faceI)
        {
            label bFaceIndex = faceI - mesh.nInternalFaces();
            label cOwn = own[faceI];
            if (nbrCellLevel[bFaceIndex] == k && cellLevel[cOwn] == -1) {
                cellLevel[cOwn] = k + 1;
                nChangedLocal++;
            }
        }

        label nGlobalChanged = nChangedLocal;
        reduce(nGlobalChanged, sumOp<label>());
        
        if (nGlobalChanged == 0) break;
    }

    // 5. Core Identification & Virtual Obstacle Marking
    // ----------------------------------------------------------------------
    // Cells unreachable by BFS (Level == -1) are considered the "Core".
    // These are treated as obstacles for the background mesh, but remain
    // fluid for this local mesh (Porous Body Logic).
    
    label nCoreCells = 0;
    forAll(cellLevel, cellI)
    {
        // If cell is deep inside the mesh (Core Region)
        if (cellLevel[cellI] == -1)
        {
            nCoreCells++;

            // NOTE: Do NOT modify patchCellTypes[...]. 
            // We retain the local status as OTHER (Calculated) to ensure 
            // fluid/porous equations are solved on this mesh.

            // Mark ONLY the Voxel Map as PATCH.
            // This signals the background mesh to treat this region as a HOLE.
            treeBoundBox cBb = cellBb(mesh, cellI);
            cBb.min() -= smallVec;
            cBb.max() += smallVec;

            if (bb.overlaps(cBb))
            {
                fill(patchTypes, bb, nDivs, cBb, patchCellType::PATCH);
            }
        }
    }

    // Report statistics
    label nGlobalCore = nCoreCells;
    reduce(nGlobalCore, sumOp<label>());

    if (Pstream::master())
    {
        Info<< "    [HoleCut] Virtual Obstacle applied. " 
            << nGlobalCore << " core cells marked as PATCH in voxel map." << endl;
    }
}
...
(
    ...
    const label nPaddingLayers  // [Modified]
)
{
...
Foam::treeBoundBox Foam::cellCellStencils::inverseDistancePadding::cellBb
(
...
bool Foam::cellCellStencils::inverseDistancePadding::overlaps
(
    const boundBox& bb,
    const labelVector& nDivs,
...
void Foam::cellCellStencils::inverseDistancePadding::markPatchesAsHoles
(
...
bool Foam::cellCellStencils::inverseDistancePadding::betterDonor
(
...
void Foam::cellCellStencils::inverseDistancePadding::markDonors
(
...
//void Foam::cellCellStencils::inverseDistancePadding::uncompactedRegionSplit
//(
//    const fvMesh& mesh,
...
void Foam::cellCellStencils::inverseDistancePadding::findHoles
(
    const globalIndex& globalCells,
    const fvMesh& mesh,
    const labelList& zoneID,
...
void Foam::cellCellStencils::inverseDistancePadding::seedCell
(
    const label cellI,
...
void Foam::cellCellStencils::inverseDistancePadding::walkFront
(
...
void Foam::cellCellStencils::inverseDistancePadding::stencilWeights
(
    const point& sample,
...
void Foam::cellCellStencils::inverseDistancePadding::createStencil
(
    const globalIndex& globalCells
)
{
...
Foam::cellCellStencils::inverseDistancePadding::inverseDistancePadding
(
    const fvMesh& mesh,
...
bool Foam::cellCellStencils::inverseDistancePadding::update()
{
    scalar layerRelax(dict_.getOrDefault("layerRelax", 1.0));

    // [Modified] 读取 paddingLayers 参数，默认值为 5 (你可以根据需要修改默认值)
    const label paddingLayers = dict_.getOrDefault<label>("holeLayers", 5);
    
    Info<< indent<< "Padding Layers:" << paddingLayers << endl;
    ...
    // ======================================================================
    // Object Registry Registration: cellTypes
    // ----------------------------------------------------------------------
    // Expose internal cellTypes_ (label) as a volScalarField named "cellTypes"
    // to the mesh registry. This enables access by external cleaning utilities
    // (e.g., cleanOversetHoles) without modifying the library interface.
    // ======================================================================
    {
        volScalarField* cellTypesPtr = nullptr;

        // 1. Check if the field already exists in the object registry
        if (mesh_.foundObject<volScalarField>("cellTypes"))
        {
            cellTypesPtr = &const_cast<volScalarField&>(
                mesh_.lookupObject<volScalarField>("cellTypes")
            );
        }
        else
        {
            // 2. If not found, create a new transient field
            cellTypesPtr = new volScalarField
            (
                IOobject
                (
                    "cellTypes",
                    ...
                    IOobject::NO_READ,
                    IOobject::NO_WRITE,
                    false // Do not register immediately, store() does it below
                ),
                mesh_,
                dimensionedScalar(dimless, Zero)
            );

            // Transfer ownership to the mesh registry
            cellTypesPtr->store();

            // Log registration event (Master only)
            if (Pstream::master())
            {
                Info<< "    [Overset Registry] 'cellTypes' field exposed to mesh database." << endl;
            }
        }

        // 3. Update field values (Type casting: label -> scalar)
        // Perform assignment in a unified loop for both existing and new fields
        volScalarField& volCellTypes = *cellTypesPtr;
        
        forAll(volCellTypes, i)
        {
            volCellTypes[i] = static_cast<scalar>(cellTypes_[i]);
        }
    }
    // ======================================================================
